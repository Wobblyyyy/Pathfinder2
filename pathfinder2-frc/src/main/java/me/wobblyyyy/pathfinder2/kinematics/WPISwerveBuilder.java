...
public class WPISwerveBuilder {
    ...
    public WPISwerveBuilder(double p, double i, double d, boolean isOptimized) {
        setCoefficients(p, i, d);

        this.isOptimized = isOptimized;
    }

    public WPISwerveBuilder(double p, double i, double d) {
        setCoefficients(p, i, d);
    }
    ...
    public WPISwerveBuilder setCoefficients(double p, double i, double d) {
        this.p = p;
        this.i = i;
        this.d = d;
    ...
    private void checkCoefficients() {
        if (Double.isNaN(p)) throw new RuntimeException(
            "proportional coefficient was not set!"
        );
        if (Double.isNaN(i)) throw new RuntimeException(
            "integral coefficient was not set!"
        );
        if (Double.isNaN(d)) throw new RuntimeException(
            "derivative coefficient was not set!"
        );
    }
    ...
    public WPISwerveChassis build() {
        ...
        if (modules.length < 3) throw new RuntimeException(
            "must have at least 3 swerve modules!"
        );

        for (int i = 0; i < modules.length; i++) modules[i] =
            new WPISwerveModule(
                turnMotors.get(i),
                turnEncoders.get(i),
