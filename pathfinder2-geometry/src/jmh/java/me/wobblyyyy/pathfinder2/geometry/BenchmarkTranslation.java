...
public class BenchmarkTranslation {
    ...
    private static void forTranslation(Consumer<Translation> consumer) {
        for (Translation a : translations) consumer.accept(a);
    }

    private static void forTranslationCombos(
        BiConsumer<Translation, Translation> consumer
    ) {
        for (Translation a : translations) for (Translation b : translations) consumer.accept(
            a,
            b
        );
    }
